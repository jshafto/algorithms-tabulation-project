...
function stepper(nums) {
    ...
    for (let i = (table.length-1); i >= 0; i-- ) {
        ...
        // start from end
        //console.log(nums.slice(i, i + nums[i]))

        // if the rest of the array .slice(i).length is less than array[i]
        if (nums.slice(i).length < nums[i]) {
            table[i] = true;
            //console.log(table)

        } else if (table.slice(i+1, i + +1+nums[i]).includes(true)) {
            table[i] = true;
            //console.log(table)
        ...
        // }
        // table[i] = true
        // else if there are any true vals table.slice(i,i+array[i])
            // table[i] = true
        // else table[i] = false;
...
function maxNonAdjacentSum(nums) {
    let table = new Array(nums.length).fill(null);
    table[0] = nums[0];
    table[1] = nums[1];
    for (let i = 2; i < nums.length; i ++ ) {
        for (let j = ; j <)
        if (nums[i] + table[i-2]> table[i]) {
